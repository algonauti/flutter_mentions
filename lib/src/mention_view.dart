...
class FlutterMentionsState extends State<FlutterMentions> {
  ...
  void mentionRemoverListener() {
    ...
    if (cursorPos >= 0 && controller!.value.text.length < _previousValue.length) {
      ...
      widget.mentions.forEach((mention) {
        mention.data.forEach((data) {
          ...
          for (var match in regex.allMatches(controller!.value.text)) {
            mentionLengthMap.add(LengthMap(str: displayTrigger, start: match.start, end: match.end));
          }
        });
      });

      var selectedMention =
          mentionLengthMap.firstWhereOrNull((element) => cursorPos >= element.start && cursorPos <= element.end);
      ...
      if (selectedMention != null) {
        ...
        controller!.selection = TextSelection.fromPosition(TextPosition(offset: controller!.value.text.length));
      }
    }
  ...
  Widget getOverlayWidget(double spaceFromTop) {
  ...
  bool getDisplayBelow(double spaceFromTop) {
  ...
  void showOverlay(String textBefore) {
    ...
    _suggestionTagOverlayEntry?.remove();
    setState(() {
      var top = (widget.focusNode?.offset.dy ?? 0) + painter.height;
      final displayBelow = getDisplayBelow(top);
      _suggestionTagOverlayEntry = OverlayEntry(
        builder: (context) {
          return Positioned(
            // Decides where to place the tag on the screen.
            top: displayBelow ? top : top,
            child: getOverlayWidget(top),
          );
        },
      );
    });
    overlayState!.insert(_suggestionTagOverlayEntry!);
  }
